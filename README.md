...
- **Offgrid mode** -- one-flag switch to an offgrid preset, relaying for nearby offgrid nodes; pick which client's offgrid frequency to match
...
```
esptool.py write_flash 0x0 mclite-v0.4.3.bin          # T-Deck Plus
esptool.py write_flash 0x0 mclite-watch-v0.4.3.bin    # T-Watch Ultra
```
...
```jsonc
{
  ...
  // Offgrid mode — leave the public mesh, relay for nearby offgrid nodes.
  // Also toggleable on-device (Settings -> Radio). Reboots to apply.
  "offgrid": {
    "enabled": false,                  // Off by default
    "preset": "auto",                  // Which offgrid radio settings to use:
                                       //   "auto"       nearest of 433/869/918 from your frequency, SF/BW/CR unchanged (default,
                                       //                and what MCLite has always done)
                                       //   "mc_open"    869.000 — what MeshCore Open uses
                                       //   "mc_app"     869.945 — what the official MeshCore Android app uses
                                       //   "mc_433"     433.000     "mc_918"  918.000
                                       //   "mclite_869" 869.000 + SF8/BW62.5/CR8 — a COMPLETE preset, so it works between
                                       //                devices whose normal regions differ (MCLite to MCLite only)
                                       //   "custom"     use the four fields below
    // Only read when preset is "custom". Omit a field (or set 0) to inherit your normal setting for it.
    "frequency": 869.4625,
    "spreading_factor": 11,
    "bandwidth": 125,
    "coding_rate": 5,

    // Optional: your own named presets, offered next to the built-in ones in the
    // config tool and on the device. Handy for a group that agrees its own offgrid
    // settings, or to follow an upstream change without waiting for a firmware
    // release. Max 8. Omit a field to inherit your normal setting for it.
    // A name that matches a built-in is ignored. Out-of-range values are ignored
    // too (frequency 150-960 MHz, SF 5-12, bandwidth 7.8-500 kHz, coding rate 5-8);
    // the device inherits your normal setting rather than moving somewhere you
    // did not ask for, and says so over serial.
    "presets": [
      { "name": "Club", "frequency": 869.4625, "spreading_factor": 11, "bandwidth": 125, "coding_rate": 5 }
    ]
  },
