...
inline OffgridRadio resolveOffgrid(const OffgridConfig& o, const RadioConfig& r) {
    ...
    const OffgridPreset& p = OFFGRID_PRESETS[offgridPresetIndex(o.preset)];

    if (o.preset == "custom") {
        // Each custom field is independent: set the ones you care about, leave
        // the rest to inherit. 0 means "not set" for all four.
        if (o.frequency       > 0.0f) out.frequency       = o.frequency;
        if (o.spreadingFactor > 0)    out.spreadingFactor = o.spreadingFactor;
        if (o.bandwidth       > 0.0f) out.bandwidth       = o.bandwidth;
        if (o.codingRate      > 0)    out.codingRate      = o.codingRate;
        return out;
    }
